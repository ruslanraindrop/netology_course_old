class Autocomplete {
  ...
  getMatches( text ) {
    ...
    for (let i = 0; i < this.input.options.length; i++) {
      if (this.input.options[i].textContent.toLowerCase().includes(text.toLowerCase())) {
        const word = {};
        word.text = this.input.options[i].textContent;
        word.value = this.input.options[i].value;
        result.push(word);
      }
    }
    return result;
